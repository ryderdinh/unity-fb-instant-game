let lib = {
	...
	$wrapHooks: {
		...
		createTournamentServer: (id, title, startTime, endTime, score) => {
			...
			fetch('https://watermelon-drop-api.bravechain.co/public/tournaments', {
				method: 'POST',
				body: JSON.stringify({
					id_tour: id,
					tour_name: title,
		...
		updateUserScoreTournamentServer: score => {
			...
			headers.append('Content-Type', 'application/json')
			fetch(
				'https://watermelon-drop-api.bravechain.co/public/tournaments/users',
				{
					method: 'POST',
					body: JSON.stringify({
	...
	openFollowOfficialPage: function () {
		FBInstant.community.canFollowOfficialPageAsync().then(() => {
			...
			window.localStorage.setItem(
				'x-qswatermelon-a2u-v6buukdhva-as',
				Number(
					window.localStorage.getItem('x-qswatermelon-a2u-v6buukdhva-as') || 0
				) + 1
			)
		})
	},
